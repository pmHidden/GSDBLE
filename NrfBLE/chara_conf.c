...
uint32_t chara_conf_add(my_service_t *p_cus) {
  ...
  BLE_GAP_CONN_SEC_MODE_SET_OPEN(&secu.write_perm);
  ...
  return sd_ble_gatts_characteristic_add(p_cus->service_handle, &char_md, &attr_char_value, &p_cus->chara_conf_handles);
}
...
uint32_t chara_conf_update(my_service_t *p_cus, chara_conf_packet_t new_value) {
  ...
  gatts_value.len = sizeof(chara_conf_packet_u);
  gatts_value.offset = 0;
  chara_conf_packet_u packet;
  memset(&packet, 0, sizeof(chara_conf_packet_u));
  packet.parsed = new_value;
  gatts_value.p_value = packet.bytes;

  // Update database.
  uint32_t err_code = sd_ble_gatts_value_set(p_cus->conn_handle, p_cus->chara_conf_handles.value_handle, &gatts_value);
  ...
  hvx_params.handle = p_cus->chara_conf_handles.value_handle;
