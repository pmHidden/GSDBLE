...
volatile uint64_t interrupts = 0u;
...
#define DATAS_PER_TRANSACTION (PACKETS_PER_TRANSACTION * DATAS_PER_PACKET)
    ...
    for (uint16_t i = 0; i < PACKETS_PER_TRANSACTION; i++) {
      imu_data data;
      data.accel_x = theData[i * DATAS_PER_PACKET].i16bit[0];
      data.accel_y = theData[i * DATAS_PER_PACKET].i16bit[1];
      data.accel_z = theData[i * DATAS_PER_PACKET].i16bit[2];
      data.gyro_x = theData[i * DATAS_PER_PACKET + 1].i16bit[0];
      data.gyro_y = theData[i * DATAS_PER_PACKET + 1].i16bit[1];
      data.gyro_z = theData[i * DATAS_PER_PACKET + 1].i16bit[2];
      data.time = (uint32_t)(theData[i * DATAS_PER_PACKET + 2].u8bit[0] << 8 | theData[i * DATAS_PER_PACKET + 2].u8bit[1]);
      if (!send_data(data))
        break;
    }
  }
